...
pub fn deposit_liquidity(
    ...
    // Defining pool creation like this this allows attactkers to frontrun pool creation with bad ratios
    let pool_creation = pool_a.amount == 0 && pool_b.amount == 0;
    (amount_a, amount_b) = if pool_creation {
        // Add as is if there is no liquidity
        (amount_a, amount_b)
    } else {
        let ratio = I64F64::from_num(pool_a.amount).checked_mul(I64F64::from_num(pool_b.amount)).unwrap();
        if pool_a.amount > pool_b.amount {
            ...
            (
                amount_a,
                I64F64::from_num(amount_a).checked_mul(ratio).unwrap().to_num::<u64>()
            )
        }
    };
    ...
    // Lock some mininum liquidity on the first deposit
    if pool_creation {
